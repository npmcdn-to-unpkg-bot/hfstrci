...
class Houseprice extends CI_Controller {
    ...
    //get page ""
		...
		list($data["lat"], $data["lng"], $data["formatted_address"], $data["loctype"], $data["locality"], $data["area"], $data["canonical"], $data["bread"], $data["cnum"], $data["resultsproperties"], $data["prices"], $data["avgproptype"], $data["related"], $data["relatedproptype"], $data["proptypeinfo"], $data["saletype"], $data["filters"], $data["postcode"] )
    = $this->searchproperty($queryseachlist);
	...
	function searchproperty($searchvalue){
		...
		$res = $this->houses->returngooglelatlong(str_replace("-", " ", $searchvalue));
			...
			list($cnum, $data['results'], $data['prices'], $avgproptype  ) = $this->Housesm->makelatlongsearch($type,$lat,$lng,62,1,$filters);
